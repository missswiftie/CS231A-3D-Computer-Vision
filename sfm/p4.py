...
def estimate_initial_RT(E):
    ...
    rt=np.concatenate((
        [np.concatenate((r1,u3),axis=1)],
        [np.concatenate((r1,-u3),axis=1)],
        [np.concatenate((r2,u3),axis=1)],
        [np.concatenate((r2,-u3),axis=1)]
    ))
    return rt
...
def linear_estimate_3d_point(image_points, camera_matrices):
    ...
    for i in range(n):
        x=image_points[i][0]
        y=image_points[i][1]
        a[i*2]=x*camera_matrices[i][2]-camera_matrices[i][0]
        a[i*2+1]=y*camera_matrices[i][2]-camera_matrices[i][1]
...
def reprojection_error(point_3d, image_points, camera_matrices):
    ...
    computed_images=(camera_matrices @ point_3d).reshape(m,3)
    computed_images/=computed_images[:,-1].reshape(-1,1)
    computed_images=computed_images[:,:2]
    return (computed_images-image_points).reshape(-1)
    #raise Exception('Not Implemented Error')
...
def jacobian(point_3d, camera_matrices):
    ...
    point_3d=np.array([point_3d[0],point_3d[1],point_3d[2],1])
    computed_images=(camera_matrices @ point_3d).reshape(m,3)
    jacobian_matrix=np.zeros((2*m,3))
    for i in range(m):
        jacobian_matrix[2*i][0]=(camera_matrices[i][0][0]*computed_images[i][2]-camera_matrices[i][2][0]*computed_images[i][0])/(computed_images[i][2]**2)
        jacobian_matrix[2*i][1]=(camera_matrices[i][0][1]*computed_images[i][2]-camera_matrices[i][2][1]*computed_images[i][0])/(computed_images[i][2]**2)
        jacobian_matrix[2*i][2]=(camera_matrices[i][0][2]*computed_images[i][2]-camera_matrices[i][2][2]*computed_images[i][0])/(computed_images[i][2]**2)
        jacobian_matrix[2*i+1][0]=(camera_matrices[i][1][0]*computed_images[i][2]-camera_matrices[i][2][0]*computed_images[i][1])/(computed_images[i][2]**2)
        jacobian_matrix[2*i+1][1]=(camera_matrices[i][1][1]*computed_images[i][2]-camera_matrices[i][2][1]*computed_images[i][1])/(computed_images[i][2]**2)
        jacobian_matrix[2*i+1][2]=(camera_matrices[i][1][2]*computed_images[i][2]-camera_matrices[i][2][2]*computed_images[i][1])/(computed_images[i][2]**2)
...
def nonlinear_estimate_3d_point(image_points, camera_matrices):
    ...
    for _ in range(10):
        ...
        sub=np.matmul(np.linalg.inv(np.matmul(jacobian_matrix.T,jacobian_matrix)),np.matmul(jacobian_matrix.T,error))
        point_3d=point_3d-sub
    return point_3d
    #raise Exception('Not Implemented Error')
...
def estimate_RT_from_E(E, image_points, K):
    ...
    m2=np.matmul(K,rts)
    m1=np.matmul(K,np.eye(3,4))
    best_count=-1
    best_index=1
    current_count=0
    for i in range(len(m2)):
        current_count=0
        m=np.concatenate(([m1],[m2[i]]))
        point_3d=np.array([nonlinear_estimate_3d_point(p,m) for p in image_points])
        point_3d=np.concatenate((point_3d,np.ones((point_3d.shape[0],1))),axis=1)
        current_count+=np.sum(point_3d[:,2]>0)
        rt=rts[i]
        point_3d_transformed=np.matmul(rt,point_3d.T).T
        point_3d_transformed=np.concatenate((point_3d_transformed,np.ones((point_3d_transformed.shape[0],1))),axis=1)
        current_count+=np.sum(point_3d_transformed[:,2]>0)
        if current_count>best_count:
            best_count=current_count
            best_index=i
    return rts[best_index]
    #raise Exception('Not Implemented Error')
