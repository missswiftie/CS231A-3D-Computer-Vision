...
def factorization_method(points_im1, points_im2):
    mean1=np.mean(points_im1,axis=0)
    mean2=np.mean(points_im2,axis=0)
    points1_bar=(points_im1-mean1).T
    points2_bar=(points_im2-mean2).T
    d=np.concatenate((points1_bar[:2],points2_bar[:2]),axis=0)
    u,s,v=np.linalg.svd(d)
    s_new=np.diag([s[0],s[1],s[2]])
    u_new=u[:,:3]
    v_new=v[:3]
    return np.matmul(np.sqrt(s_new),v_new),np.matmul(u_new,np.sqrt(s_new))
