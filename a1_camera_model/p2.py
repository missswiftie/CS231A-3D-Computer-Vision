...
def compute_camera_matrix(real_XY, front_image, back_image):
    ...
    p=np.zeros((48,))
    for i in range(N):
        P[4*i][0:4]=[real_XY[i][0],real_XY[i][1],0,1] 
        P[4*i+1][4:8]=[real_XY[i][0],real_XY[i][1],0,1]
        P[4*i+2][0:4]=[real_XY[i][0],real_XY[i][1],150,1] 
        P[4*i+3][4:8]=[real_XY[i][0],real_XY[i][1],150,1]
        p[4*i]=front_image[i][0]
        p[4*i+1]=front_image[i][1]
        p[4*i+2]=back_image[i][0]
        p[4*i+3]=back_image[i][1]
    M=np.linalg.pinv(P).dot(p).reshape((2,4))
...
def rms_error(camera_matrix, real_XY, front_image, back_image):
    ...
    error=0
    for i in range(N):
        p1=np.dot(camera_matrix,np.array([real_XY[i][0],real_XY[i][1],0,1]))
        p2=np.dot(camera_matrix,np.array([real_XY[i][0],real_XY[i][1],150,1]))
        error+=np.sum((p1[:2]-front_image[i])**2)
        error+=np.sum((p2[:2]-back_image[i])**2)
    return np.sqrt(error/(2*N))
    # END YOUR CODE HERE
'''
TEST_P2
