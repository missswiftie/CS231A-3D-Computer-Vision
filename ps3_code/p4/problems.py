...
def bilinear_sampler(img, disp):
    ...
    y_coords, x_coords = torch.meshgrid(
        torch.arange(height, dtype=torch.float32, device=img.device),
        torch.arange(width, dtype=torch.float32, device=img.device),
        indexing='ij'
    )
    x_coords = x_coords / (width - 1)
    y_coords = y_coords / (height - 1)
    x_coords = x_coords.unsqueeze(0).expand(batch_size, -1, -1)  # (batch_size, height, width)
    y_coords = y_coords.unsqueeze(0).expand(batch_size, -1, -1)  # (batch_size, height, width)
    x_coords = x_coords + disp[:,0,:,:]
    grid_x = x_coords * 2 - 1
    grid_y = y_coords * 2 - 1
    grid = torch.stack((grid_x, grid_y), dim=-1)
    output = F.grid_sample(img, grid, align_corners=True, mode='bilinear')
    return output
...
if __name__ == '__main__':
    data = np.load('C:\\Users\\郑嘉祺\\Downloads\\ps3_code\\ps3_code\\p4\\data.npz')
