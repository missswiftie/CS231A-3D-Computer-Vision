...
class MNISTDatasetWrapper(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.for_rotation_classification:
            ...
            label = random.choice(self.rot_choices) # TODO choose a label for rotation
            img = img.rotate(label) # TODO use PIL's rotate function to modify the image according to the label
    ...
    def show_batch(self, n=3):
        ...
        for i in range(n):
            for j in range(n):
                ...
                if not self.for_rotation_classification:
                    axs[i, j].set_title('Label: {0} (#{1})'.format(label, self.label_to_class[label]))
                else:
                    axs[i, j].set_title('Label: {0} ({1} Degrees)'.format(label, label*45))
                axs[i, j].axis('off')
