...
def depth_loss(y_true, y_pred, theta=0.1, device="cuda"):
    ...
    # Edges 
    dy_true, dx_true = image_gradients(y_true,device=device) # TODO get the images gradients of the true data 
    dy_pred, dx_pred = image_gradients(y_pred,device=device) # TODO get the images gradients of the predicted data 
    l_edges = torch.mean(torch.abs(dy_pred-dy_true)+torch.abs(dx_pred-dx_true)) # TODO use torch to calculate the mean absolute difference between the true and predicted gradients
